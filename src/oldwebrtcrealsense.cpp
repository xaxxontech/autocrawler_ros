...
start_pipeline (void)
{
  ...
  //pipe1 =
	...
	pipe1 =
      gst_parse_launch ("webrtcbin bundle-policy=max-bundle name=sendrecv " STUN_SERVER
      "v4l2src device=/dev/video3 ! videorate ! video/x-raw,width=640,height=480,framerate=8/1 ! "
...
int main(int argc, char **argv) // added
{
	...
	image_sub_ = image_transport_->subscribe("/camera/color/image_raw", 10, image_cb); 
	// ^^ no segfault, but no ros_info output still -- maybe because no ros.spin() in effect, stuck in gst loop
	
	ros::AsyncSpinner spinner(4);
	spinner.start();
	// ^^ works
	
	ROS_INFO("webrtcrs: moving on ....");
