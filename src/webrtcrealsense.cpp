...
#include <gst/app/app.h> // << added, not part of gstwebrtc-demos git
...
static GstElement *pipe1, *webrtc1, *appsrc_;  // apprsc added 
...
//static ros::Publisher webrtcstatus_pub; // ROS1
rclcpp::Publisher<std_msgs::msg::String>::SharedPtr webrtcstatus_pub; // ROS2

std::unique_ptr<image_transport::ImageTransport> image_transport_; // ROS1/2
image_transport::Subscriber image_sub_; // ROS1/2
...
// static const gchar *ros_args= nullptr; 
...
//  { "ros-args", 0, 0, G_OPTION_ARG_STRING, &ros_args, "Dummy option so ros-args doesn't throw error", NULL },
  { NULL },
};
...
static gboolean start_pipeline (void)
{
  ...
  GError *error = NULL;
	...
	if (appsrc_ == nullptr) {
		RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "GST: failed to create appsrc!");
		return FALSE;
	}
	...
	RCLCPP_INFO(rclcpp::get_logger("rclcpp"), ("GST: inelement: %s", gst_element_get_name(inelement)));

	if (!gst_bin_add(GST_BIN(pipe1), appsrc_)) {
				RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "GST: gst_bin_add() failed!");
	...
	if (!gst_element_link(appsrc_, inelement)) {
		RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), ("GST: cannot link %s -> %s",
    ...
    gpointer user_data G_GNUC_UNUSED)
{
	...
	auto rosmsg = std_msgs::msg::String();
    rosmsg.data = "disconnected";
...
GstCaps* gst_caps_new_from_image(const sensor_msgs::msg::Image::ConstSharedPtr &msg)
{
	...
	if (msg->is_bigendian) {
		RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "GST: big endian image format is not supported");
		return nullptr;
	}

	auto format = known_formats.find(msg->encoding);
	if (format == known_formats.end()) {
		RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), ("GST: image format '%s' unknown", msg->encoding.c_str()));
		return nullptr;
	}
...
int main(int argc, char** argv)
{
	...
	rclcpp::init(argc, argv);
	std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("webrtcrs");
	...
	g_option_context_set_ignore_unknown_options(context, TRUE); // ROS2 added to ignore --ros-args
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "gstreamer plugin check OK");
	...
	* it's probably a test server with a self-signed certificate */
	...
	webrtcstatus_pub = node->create_publisher<std_msgs::msg::String>("webrtcstatus", 10);
	...
	while (rclcpp::ok())
	{
		...
		if (serverConnectStarted && start==0) start = node->now().nanoseconds()/1000000000;
		
		if (start !=0 && !serverConnected && node->now().nanoseconds()/1000000000 - start > 2) {
